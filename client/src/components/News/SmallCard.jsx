...
const SmallCard = ({ _id, title, createdAt, viewsCount, image }) => {
  return (
    <div className="flex items-center my-3">
      <img src={image.url} className="h-24 mr-3" alt="" />
      <div className="">
        <Link to={`/post/${_id}`}>
          <h1 className="font-bold leading-5 text-sm text-zinc-900">{title}</h1>
        </Link>
