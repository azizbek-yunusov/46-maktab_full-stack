...
import { deleteOrder, getOrders, selectedDeleteOrder } from "../../redux/order";
import { DownloadTableExcel } from "react-export-table-to-excel";
import { MoreMenu, NotData, SearchInput, TableButton } from "../Helpers";
import { Layout } from "../Layouts";
import GridList from "./GridList";
import TableBody from "./TableBody";
import { orderStatusData, paymentMethodData } from "../../data/OrderTypeData";
import { HelmetTitle } from "../../utils";
import ExportExcelOrderData from "./ExportExcelOrderData";
...
const OrdersList = () => {
  ...
  const filteredOrders = orders?.filter((order) => {
    ...
    if (term && !order.orderId.toLowerCase().includes(term.toLowerCase())) {
      return false;
    }
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedOrderIds = filteredOrders.map((order) => order._id);
    } else {
      newSelectedOrderIds = [];
    }
          ...
          <div className="bg-white relative dark:bg-[#2e2d4a] rounded-lg overflow-hidden my-6 border border-gray-200 dark:border-gray-600">
              ...
              <div className="grid grid-cols-3 gap-x-5 pb-6 mb-3 px-5 border-b border-b-gray-200 dark:border-b-gray-600">
                <FormControl size="medium" sx={{}}>
                  ...
                  >
                    ...
                    <MenuItem value={"new"}>{t("new-orders")}</MenuItem>
                    <MenuItem value={"old"}>{t("old-orders")}</MenuItem>
                  </Select>
                </FormControl>
                <FormControl size="medium" sx={{}}>
                  <InputLabel _id="demo-simple-select-label">
            ...
            {selectedOrderIds.length ? (
              ...
              <div className="flex w-full items-center justify-between py-3 px-4">
                ...
                <div className="flex items-center">
                  <DownloadTableExcel
                    filename={`${t("orders-list")}-${moment(now).format("l")}`}
                    sheet="users"
            ...
            {orders.length ? (
              <>
                ...
                <ExportExcelOrderData tableRef={tableRef} orders={orders} />
              </>
            ) : (
              <NotData string={t("not-data")} />
            )}
          </div>
        )}
      </Layout>
